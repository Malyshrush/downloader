...
async function ensureTable(client, tableName, keyName) {
...
async function main() {
  ...
  const idempotencyTable = await ensureTable(dynamoClient, idempotencyTableName, 'eventId');
  const hotStateTable = await ensureTable(dynamoClient, hotStateTableName, 'objectKey');
