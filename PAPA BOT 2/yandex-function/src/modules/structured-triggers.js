...
async function processStructuredTriggers(data, profileId = '1') {
    ...
    const rows = await getSheetData('ТРИГГЕРЫ', details.communityId, profileId);
    if (!rows || !rows.length) {
        return { matched: false, handled: false };
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if ((row['Ответить если в Группе'] || '').trim() && details.userId) {
            const groupsMatched = await checkUserGroups(details.userId, row['Ответить если в Группе'], details.communityId, profileId);
            if (!groupsMatched) {
        ...
        await recordStructuredTriggerExecution(profileId, details.communityId);
        handledAny = true;
