...
async function handleMessage(data, profileId = '1') {
    try {
        ...
        if (matchedRowWithConditions) {
            ...
            await sendMessageAndPerformActions(userId, matchedRowWithConditions, message, false, communityId, profileId);
        } else if (matchedRowWithoutConditions) {
            log('debug', '🚀 STEP 4.2: SENDING FALLBACK FROM TRIGGER ROW');
            await sendFallbackResponseFromRow(userId, matchedRowWithoutConditions, message, communityId, profileId);
        } else if (fallbackRow) {
            log('debug', '🚀 STEP 4.3: SENDING GENERAL FALLBACK');
            await sendFallbackResponseFromRow(userId, fallbackRow, message, communityId, profileId);
        } else {
