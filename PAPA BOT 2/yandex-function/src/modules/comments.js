...
async function handleComment(data, profileId = '1') {
    try {
        ...
        if (matchedRowWithConditions) {
            await sendCommentAndPerformActions(comment, groupId, matchedRowWithConditions, communityId, profileId);
        } else if (matchedRowWithoutConditions) {
            await sendFallbackCommentFromRow(comment, groupId, matchedRowWithoutConditions, communityId, profileId);
        } else if (fallbackRow) {
            await sendFallbackCommentFromRow(comment, groupId, fallbackRow, communityId, profileId);
        }
    } catch (error) {
