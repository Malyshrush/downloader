...
const GROUP_FIELD = '\u0413\u0440\u0443\u043f\u043f\u0430';
const USER_GROUP_FIELD = '\u0413\u0420\u0423\u041f\u041f\u0410';
const MINIAPP_ENABLED_FIELD = 'MiniApp \u0432\u043a\u043b\u044e\u0447\u0435\u043d';
const MINIAPP_TITLE_FIELD = 'MiniApp \u0437\u0430\u0433\u043e\u043b\u043e\u0432\u043e\u043a';
const YES_VALUE = '\u0434\u0430';
...
test('Mini App detail marks group subscribed from signed launch query', async () => {
    ...
    const response = await __testOnly.handleMiniAppRequestWithDependencies({
        ...
        queryStringParameters: {
            ...
            g: 'vip',
            ...launch,
            sign: signVkLaunchParams(launch, secret)
        }
    }, {
        miniAppSecret: secret,
        getSheetData: async () => [
            { [GROUP_FIELD]: 'vip', [MINIAPP_ENABLED_FIELD]: YES_VALUE, 'MiniApp slug': 'vip', [MINIAPP_TITLE_FIELD]: 'VIP' }
        ],
        resolveCommunity: async () => ({ communityId: '229445618', profileId: '1' }),
        getUserRow: async () => ({ [USER_GROUP_FIELD]: 'vip' })
    });

    assert.equal(response.statusCode, 200);
    assert.equal(parse(response).group.subscribed, true);
});

test('Mini App group list marks subscribed items from signed launch query', async () => {
    const secret = 'miniapp-secret';
    const launch = { vk_app_id: '999', vk_user_id: '123', vk_group_id: '229445618' };
    const response = await __testOnly.handleMiniAppRequestWithDependencies({
        httpMethod: 'GET',
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(parse(response).groups[0].subscribed, true);
});

test('Mini App read API ignores invalid launch query subscription state', async () => {
    const response = await __testOnly.handleMiniAppRequestWithDependencies({
        httpMethod: 'GET',
        queryStringParameters: {
            miniapp: 'group',
        ...
        resolveCommunity: async () => ({ communityId: '229445618', profileId: '1' }),
        getUserRow: async () => {
            throw new Error('should not read user on invalid sign');
        }
    });

    assert.equal(response.statusCode, 200);
    assert.equal(parse(response).group.subscribed, false);
});
