...
function buildEventRuntimeConfig(env = process.env) {
  ...
  return {
    ...
    ydbMessageRulesTable: String(env.YDB_MESSAGE_RULES_TABLE || 'message_rule_entries').trim() || 'message_rule_entries',
    ydbCommentRulesTable: String(env.YDB_COMMENT_RULES_TABLE || 'comment_rule_entries').trim() || 'comment_rule_entries',
    ...
    allowAllLegacySheetFallback: String(env.ALLOW_LEGACY_SHEET_FALLBACK || '').trim(),
    allowLegacySchedulerSheetAccess: String(env.ALLOW_LEGACY_SCHEDULER_SHEET_ACCESS || '').trim(),
    allowLegacyVariablesSheetAccess: String(env.ALLOW_LEGACY_VARIABLES_SHEET_ACCESS || '').trim(),
    allowLegacyAppLogsSheetAccess: String(env.ALLOW_LEGACY_APP_LOGS_SHEET_ACCESS || '').trim(),
