...
// Разрешаем CORS для вашей админ-панели (Yandex Cloud Functions)
app.use((req, res, next) => {
  res.header('Access-Control-Allow-Origin', 'https://functions.yandexcloud.net'); // или ваш домен
...
async function uploadPhotoToMessages(userToken, groupId, buffer, filename) {
  const uploadRes = await axios.get('https://api.vk.com/method/photos.getMessagesUploadServer', {
    params: { group_id: groupId, access_token: userToken, v: '5.199' }
  });
  ...
  const saveRes = await axios.post('https://api.vk.com/method/photos.saveMessagesPhoto', null, {
    params: { server, photo, hash, group_id: groupId, access_token: userToken, v: '5.199' }
  });
...
async function uploadDocToMessages(userToken, groupId, buffer, filename) {
  // Получаем upload URL для документов (без group_id)
  ...
  const peerId = 2000000000 + Math.abs(groupId);
...
async function uploadVideoToWall(userToken, groupId, buffer, filename) {
  // Для стены используем тот же video.save с group_id (на стене видео отображается)
